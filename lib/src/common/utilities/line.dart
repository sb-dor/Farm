...
class Line extends Equatable {
  ...
  factory Line.doubles(double startX, double startY, double endX, double endY) =>
      Line(Vector2(startX, startY), Vector2(endX, endY));

  factory Line.fromPosition({required Vector2 startingPosition, required Vector2 delta}) =>
  ...
  Vector2? intersectsAt(Line other) {
    ...
    double t =
        (other.dx * (start.y - other.start.y) - other.dy * (start.x - other.start.x)) /
        (-other.dx * dy + dx * other.dy);
